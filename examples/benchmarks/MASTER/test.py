...
# 确保能够获取到Qlib官方提供的get_data.py数据下载脚本, 确保后续可以通过get_data.py下载Qlib的金融数据
...
logger.warning(f"模型创建完成: {model}")

# 创建数据集
dataset = init_instance_by_config(config['task']["dataset"])
logger.warning(f"数据集创建完成: {dataset}")
...
with R.start(experiment_name="train_model"):
    try:
        ...
        logger.warning(f"模型载入完成: {dataset}")
...
# 获取测试集标签
# testset = dataset.prepare(segments="test", col_set="feature_label")
...
print(pred_label.dtypes)
print(pred_label.index.names)
